...
particles_value_personal_best = rand(particle_grid_steps, particle_grid_steps, values) ...
                              + (j * rand(particle_grid_steps, particle_grid_steps, values));
particle_personal_best_objective_function = zeros(particle_grid_steps, particle_grid_steps, values);

swarm_global_best_value = ones(1, 1, values);
swarm_global_best_objective_function = zeros(1, 1, values);
   ...
   for value = 1:1:values
      for n = 1:1:particle_grid_steps
         for nn = 1:1:particle_grid_steps
            ...
            SINR = sum(F(end, find(theta_vector_degrees == beam_theta)) - F(end, find(ismember(theta_vector_degrees, null_thetas)))) / length(null_thetas);
            Beam_Power = F(end, find(theta_vector_degrees == beam_theta));
            %Compute objective function END            
            
            objective_function_value = (SINR/Goal_SINR_Value) + (Beam_Power / Goal_Beam_Power);
